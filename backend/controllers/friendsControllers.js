...
const addFriend = async (req, res) => {
    try {
      ...
      if (userId.toString() === friendId) throw new Error("You can't add yourself.");
      ...
      if (friend.friendRequests?.includes(userId)) throw new Error("Friend request already sent.");
...
const acceptFriendRequest = async (req, res) => {
  try {
    ...
    if (!user.friendRequests.includes(senderId)) throw new Error("No such friend request.");
...
const removeFriend = async (req, res) => {
  try {
    ...
    res.status(200).send({ msg: { title: "Friend removed." } });
  } catch (error) {
    res.status(400).send({ msg: { title: error.message } });
  }
};
...
const getAllFriends = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate("friends", "_id name email");
...
const getFriendRequests = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate("friendRequests", "_id name email");
...
const getSentFriendRequests = async (req, res) => {
    try {
      const user = await User.findById(req.user._id).populate("sentRequests", "_id name email");
